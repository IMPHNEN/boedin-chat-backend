...
pub fn prepare_env_file(path: PathBuf) -> std::io::Result<()> {
    let vars = vec![
        ...
        ("CHANNEL_CAPACITY", "50"),
        ("-", "-"),
        ("JWT_SECRET", "imphnen_most_secret_key"),
        ("JWT_SECRET_REFRESH", "imphnen_most_secret_refresh_key"),
        ("-", "-"),
        ("BACKEND_URL", "https://budin.azumidev.web.id"),
        ("FRONTEND_URL", "https://boedinchat.vercel.app"),
        ("DATABASE_URL", "sqlite:imphnen.db"),
        ("-", "-"),
        ("DISCORD_GUILD_ID", "1206212469148749834"),
        ("DISCORD_CLIENT_ID", "1331832116270338140"),
